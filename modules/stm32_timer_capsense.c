...
static void test_task(void *p) {
	Stm32TimerCapsense *self = (Stm32TimerCapsense *)p;

	float value = 0;

	while (1) {
		interface_sensor_value(&self->probe, &value);
		u_log(system_log, LOG_TYPE_INFO, U_LOG_MODULE_PREFIX("%u"), (uint32_t)value);
...
stm32_timer_capsense_ret_t stm32_timer_capsense_init(Stm32TimerCapsense *self, uint32_t timer, uint32_t probe_reset_oc, uint32_t probe_ic) {
	...
	xTaskCreate(test_task, "capsense", configMINIMAL_STACK_SIZE + 128, (void *)self, 1, NULL);
